...
export const getConfig = async (): Promise<Dashboard[]> => {
  if (process.env.EDGE_CONFIG) {
    const dashboards = await get(process.env.CONFIG_KEY ?? 'dashboards') as Dashboard[];
    if (!dashboards) {
  ...
  await configSchema.validate(data[process.env.CONFIG_KEY ?? 'dashboards']);
  return data[process.env.CONFIG_KEY ?? 'dashboards'];
};